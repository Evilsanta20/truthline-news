...
CREATE INDEX IF NOT EXISTS idx_news_fetch_logs_timestamp ON public.news_fetch_logs(fetch_timestamp);
CREATE INDEX IF NOT EXISTS idx_article_archive_original_id ON public.article_archive(original_article_id);
...
-- Create trigger for archiving (drop first if exists)
DROP TRIGGER IF EXISTS archive_article_trigger ON public.articles;
